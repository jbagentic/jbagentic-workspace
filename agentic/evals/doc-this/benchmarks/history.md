...
| 3 | 2026-06-09 | claude-opus-4-8 | case 5 PASS + cases 3,4 no regression | case 5 PASS (old skill) | — | — | — | Widened the source to support change-driven reconcile. New case 5 (reconcile after a stated change, folder named) PASSes with-skill — no material-ask, no approval round-trip, minimal/idempotent. Old-skill baseline also reconciles once invoked, so case 5 does **not** discriminate on execution; the win is triggering (description), tested separately. Gate cases 3 & 4 still PASS — widening the source didn't break the input gates. Manual harness, 1 run/case. |
