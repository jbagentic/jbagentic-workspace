#!/usr/bin/env python3
"""Run the context-maintenance eval executors headlessly and capture transcripts.

For every staged run dir (prepare.py wrote a seed.json into each), launch one
`claude -p` process with cwd = the run's $TMPDIR clean room. Unlike the discovery
eval this is WRITE-ENABLED: the agent must perform the requested code change, so
Edit/Write are whitelisted alongside Read/Grep/Glob and Skill (the doc-this skill
is copied into the room). Bash is explicitly disallowed so an executor can't shell
out — all mutation goes through Edit/Write, which grade.py reads back from the
clean room. Mutations are contained: the room is a throwaway $TMPDIR copy.
...
Usage:
  python3 run.py runs/iteration-1 [--jobs 4] [--timeout 360] [--model <id>]
"""
import argparse
import json
import subprocess
...
ALLOWED_TOOLS = "Read,Edit,Write,Grep,Glob,Skill"
...
def run_one(run_dir: Path, timeout: int, model: str | None):
    ...
    detail = f"{tokens} tok, {duration_s}s" if ok else f"rc={proc.returncode}"
