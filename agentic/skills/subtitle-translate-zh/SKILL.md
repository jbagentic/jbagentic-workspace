...
- Companion materials when present — consulted only to disambiguate terms, not as a default opener. Prefer the slide deck's pre-extracted text when the project provides one (e.g. `slides.txt`) over the slide PNGs/PDF; also README or abstract.
...
1. **Translate to Simplified Chinese.** Produce `.zh-Hans.srt` entry by entry, in the vocabulary the target audience would naturally use. Consult companion materials (prefer the slide deck's pre-extracted text when available) only when you hit a term whose meaning is genuinely ambiguous from the SRT alone.
