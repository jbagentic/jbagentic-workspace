...
- Its domain context for the glossary. Prefer a `slides.txt` (the deck's extracted text — cheap to read) when present; otherwise fall back to the slide-deck PDF, slide PNGs, abstract, or README. Discover alongside the subtitle file per the project's layout; ask only if nothing is found.
