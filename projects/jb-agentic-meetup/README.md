...
1. **[Pre-edit prep](docs/talk-pre-edit-prep.runbook.md)** — *agent.* `slides.pdf` → `slides/slide-NN.png` overlay frames.
2. **[Video edit (CapCut)](docs/talk-video-edit.runbook.md)** — *human, manual.* Recorded video + overlays → final video + raw English subtitles (`.en.raw.srt`).
3. **[Post-edit processing](docs/talk-post-edit-processing.runbook.md)** — *agent.* `.en.raw.srt` → polished + translated subtitles and `youtube-content.md`, ready to upload.
