...
1. **[Pre-edit prep](docs/pre-edit-prep.runbook.md)** — *agent.* `slides.pdf` → `slides/slide-NN.png` overlay frames.
2. **[Video edit (CapCut)](docs/video-edit.runbook.md)** — *human, manual.* Recorded video + overlays → final video + raw English subtitles (`.en.raw.srt`).
3. **[Post-edit processing](docs/post-edit-processing.runbook.md)** — *agent.* `.en.raw.srt` → polished + translated subtitles and `youtube-content.md`, ready to upload.
...
- [Pre-edit prep runbook — render slides.pdf into slide-NN.png overlay frames](docs/pre-edit-prep.runbook.md)
- [Video edit (CapCut) runbook — manual edit handoff: inputs needed and outputs to produce](docs/video-edit.runbook.md)
- [Post-edit processing runbook — polish and translate subtitles, then write YouTube metadata](docs/post-edit-processing.runbook.md)
