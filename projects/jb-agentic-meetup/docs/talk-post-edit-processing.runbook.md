...
**Actor:** agent · **In:** `<talk-slug>.en.raw.srt` · `slides/` · the Drive video link · **Out:** polished + translated SRTs, `youtube-content.md`, `video-file.html`
...
2. **Polish the English subtitles** — run `/subtitle-polish`. Writes the polished file with `.raw` dropped from the stem.
   - In: `<talk-slug>.en.raw.srt` (source) · `slides/` (glossary context)
